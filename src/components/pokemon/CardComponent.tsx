...
import { getGradientStyle, baseColors, formatNumber } from '@/lib/utils';
import {
    Tooltip,
    TooltipContent,
    TooltipProvider,
...
const CardComponent: React.FC<Pokemon> = ({ url, name }) => {
    ...
    return (
        <>
            {
                !isLoading && <div onClick={handleOnClickDetail} className="rounded-md shadow-lg p-5 overflow-hidden transition-all cursor-pointer border-2 border-transparent hover:border-black dark:hover:border-white" style={gradientStyle}>
                    <div className="flex flex-col lg:flex-row relative h-full items-center justify-center">
                        ...
                        <div className='flex-1 flex flex-col z-10 order-2 lg:order-1 self-stretch'>
                            ...
                            <div className='mt-auto mb-2'>
                                {data?.types.map((item: { type: { name: string } }, index: number, array: []) => <span className='italic text-sm dark:text-gray-300' key={index}>{array.length === index + 1 ? item.type.name : (item.type.name + ',')} </span>)}
                            </div>
            ...
            {isLoading && <div className='rounded-md bg-gray-300 animate-pulse p-5 overflow-hidden'>
